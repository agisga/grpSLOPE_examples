...
TestGroupSLOPE <- function(n.significant, n.subjects, n.predictors, fdr, verbose=FALSE){
  ...
  # where Sigma has entries (0.5)^|i-j| for
  # a pair of predictors from ith and jth groups where i!=j, 
  # and 0.7 if i=j. 
  Sigma <- diag(rep(0.7, n.group))
  for (i in 1:(n.group-1)) {
    for (j in (i+1):n.group) {
...
save(list = ls(all.names = TRUE), file = "test_lambdaMC_methods_weak_signal_cov_AR1_50perc.RData")
...
postscript(file="gaussianMC_weak_signal_cov_AR1_50perc.eps", horizontal=FALSE, width=400, height=400)
...
postscript(file="chiMC_weak_signal_cov_AR1_50perc.eps", horizontal=FALSE, width=400, height=400)
...
postscript(file="chiMean_weak_signal_cov_AR1_50perc.eps", horizontal=FALSE, width=400, height=400)
...
postscript(file="selected_by_lambdaMC_weak_signal_cov_AR1_50perc.eps", horizontal=FALSE, width=400, height=400)
